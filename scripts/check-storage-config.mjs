...
try {
  const environments = selectStorageEnvironments(process.argv.slice(2));
  const rc = JSON.parse(await readFile(new URL("../.firebaserc", import.meta.url), "utf8"));
  const projects = validateFirebaseProjects(rc);
  const policy = JSON.parse(await readFile(new URL("../firebase/storage-policy.json", import.meta.url), "utf8"));
  validateStoragePolicy(policy);
  for (const name of ["FIREBASE_TOKEN", "GOOGLE_APPLICATION_CREDENTIALS", "FIREBASE_STORAGE_EMULATOR_HOST", "STORAGE_URL"]) {
  ...
  await requireAuth({ project: projects[environments[0]], projectRoot: root, nonInteractive: true, ...account }, true);
  const client = new Client({ urlPrefix: "https://storage.googleapis.com/storage/v1", auth: true });

  for (const environment of environments) {
    console.log(`\nStorage ${environment}: ${projects[environment]} (solo lectura)`);
    try {
      const remote = await readRemoteStorageBuckets(client, projects[environment]);
      const result = assessStorageBuckets(remote, projects[environment], policy);
      ...
      console.error(`No se encontró un bucket verificable${Number.isInteger(status) ? ` (HTTP ${status})` : ""}. Revisá plan, permisos y activación en Firebase Console.`);
      process.exitCode = 1;
    }
  }
  console.log("\nEste comando no crea buckets, objetos ni reglas.");
} catch (error) {
  if (error.code === "MODULE_NOT_FOUND") console.error("Faltan dependencias: ejecutá npm.cmd ci --ignore-scripts.");
    ...
    ? error.message : "No se pudo preparar el chequeo Storage. Revisá configuración, conexión y sesión Firebase CLI.");
  process.exitCode = 1;
}
